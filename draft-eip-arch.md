...
The GOB Schema is interpreted within the context of the IOAM-Namespace carried by the option. The GOB-based integration therefore inherits the IOAM-Namespace machinery of {{RFC9197}}: an EIP deployment can map an EIP domain to an IOAM Namespace-ID from the operator-assigned range, obtaining namespace-based processing gating (nodes that are not configured for the namespace do not modify the option), disambiguation when multiple IOAM options are present due to overlapping or layered domains, and removal of the option at domain boundaries. The standalone EIP carriers (Hop-by-Hop Option and SRH TLV) do not define an in-packet domain identifier. Instead, they rely on the controlled domain model of {{RFC8799}}: the operator is responsible for confining EIP options to the domain, or to the set of cooperating domains, e.g., by filtering at domain boundaries or by carrying EIP within an edge-to-edge encapsulation. Within this model, EIP Information Element codes from the IANA-assigned subranges have global semantics and can be interpreted anywhere, while operator-defined codes are meaningful only within the controlled domain that configured them (see {{id-eip-headers}}). The IOAM/GOB-based carrier additionally provides the in-packet IOAM-Namespace context described above.
